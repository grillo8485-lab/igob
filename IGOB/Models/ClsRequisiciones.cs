...
namespace IGOB.Models
{
    public class ClsRequisiciones
    {
        ...
        public bePresupuestosPartidas getPresupuestosPartidas(int pIdDependencia,int pIdPartida)
        {
            brPresupuestosPartidas b = new brPresupuestosPartidas();
            var c = b.PresupuestosPartidas_Traer_IdDependencia_IdPartida(pIdDependencia, pIdPartida);
            return c;
        }
        ...
        public List<Carta> getAllCartas_x_IdTipoSolicitud(int pIdTipoSolicitud,int pIdrequisicion)
        {
            List<Carta> cartas = new List<Carta>();

            var lstCartas = getAllCartas(pIdrequisicion).Where(s => s.IdTipoSolicitud == pIdTipoSolicitud);
        ...
        protected List<beCartas> getAllCartas(int pIdrequisicion)
        {
            List<beCartas> lstCartas = (new brCartas()).listar_Cartas_x_IdRequisicion(pIdrequisicion).ToList();
            return lstCartas;
        }
        ...
        public int updateRequision(beRequisiciones requisiciones)
        {

            brRequisiciones b = new brRequisiciones();
            return b.actualizarRequisiciones(requisiciones);
        ...
        public List<ClsGenerica> getAllCondicionesPagoDetalle_x_IdCondicionPago(int IdCondicionPago)
        {
            ...
            foreach (var item in sf)
            {
                ...
                a.string1 = CultureInfo.CurrentCulture.TextInfo.ToTitleCase(MonthName(item.FechaPago.Month)) + " - " + item.FechaPago.Year.ToString();
                a.string2 = "";
        ...
        public int saveRequisicionFirmante(beRequisicionesFirmantes ReqFirmantes)
        {
            return (new brRequisicionesFirmantes()).actualizarRequisicionesFirmantes(ReqFirmantes);
        }
        public int saveRequisicionCarta(int pIdCarta, int pIdRequisicion,int pIdUsuarioRegistro)
        {
            beRequisicionesCartas requisicionesCartas = new beRequisicionesCartas();
            requisicionesCartas.IdProveedorRequisicionCarta =0;
            requisicionesCartas.IdRequisicion = pIdRequisicion;
            requisicionesCartas.IdCarta = pIdCarta;
            requisicionesCartas.IdUsuarioRegistro = pIdUsuarioRegistro;
            requisicionesCartas.FechaRegistro = DateTime.Now;
            requisicionesCartas.Autorizada = 0;
            return (new brRequisicionesCartas()).guardarRequisicionesCartas(requisicionesCartas);
        }

        public beManifiestosProveedores getManifiestosProveedores_x_IdProveedorRqInvitacion(int pIdProveedorRqInvitacion)
        {
            var RequisicionCondiconesDeEntrega = (new brManifiestosProveedores()).traerManifiestoProveedore_x_IdProveedorRqInvitacion(pIdProveedorRqInvitacion);
        ...
        {
            var lstManifiestoProveedoresClientes = (new brManifiestoProveedoresClientes()).getAllManifiestosProveedores_x_IdManifiestoProveedor(pIdManifiestoProveedor);
        ...
        {
            var lstManifiestoProveedoresDeclaratorias = (new brManifiestoProveedoresDeclaratorias()).getAllManifiestoProveedoresDeclaratorias_x_IdManifiestoProveedor(pIdManifiestoProveedor);

            return lstManifiestoProveedoresDeclaratorias;
        }
        public List<beRequisicionLotesIdRequisicion> getAllRequisicionLotes_x_IdProveedorRqInvitacion(int pIdProveedorRqInvitacion)
        {
            var lstRequisicionLotes = (new brRequisicionesLotes()).traerRequisicionesLotes_x_IdProveedorRqInvitacion(pIdProveedorRqInvitacion);

            return lstRequisicionLotes;
        }
        public int savePropuestasTecnicaEconomicaLotes(bePropuestasTecnicaEconomicaLotes PropuestasTecnicaEconomicaLotes)
        {
            var a = (new brPropuestasTecnicaEconomicaLotes()).actualizarPropuestasTecnicaEconomicaLotes(PropuestasTecnicaEconomicaLotes);

            return a;
        }
        public bePropuestasTecnicaEconomicaLotes getPropuestasTecnicaEconomicaLotes(int pIdPropuestasTecnicaEconomicaLotes)
        {
            var PropuestasTecnicaEconomicaLotes = (new brPropuestasTecnicaEconomicaLotes()).traerPropuestasTecnicaEconomicaLotes_x_IdPropuestaTecnicaEconomica(pIdPropuestasTecnicaEconomicaLotes);

            return PropuestasTecnicaEconomicaLotes;
        }
        public List<Carta> listar_Cartas_x_IdProveedorRqInvitacion(int pIdProveedorRqInvitacion)
        {
            var PropuestasTecnicaEconomicaLotes = (new brCartas()).listar_Cartas_x_IdProveedorRqInvitacion(pIdProveedorRqInvitacion);
            List<Carta> lstCarta = new List<Carta>();
            foreach(var item in PropuestasTecnicaEconomicaLotes)
            {
                Carta a = new Carta();
                a.IdCarta = item.IdCarta;
                a.Numero = item.Numero;
                ...
                a.Inciso = item.Inciso;
                a.CartaTexto = item.Carta;
                a.IdProveedorCarta = item.IdProveedorCarta;
                a.IdProveedorRqInvitacion = item.IdProveedorRqInvitacion;
                a.FechaAceptacion = item.FechaAceptacion;
                a.AceptacionCarta = item.AceptacionCarta;
                lstCarta.Add(a);
            }
            return lstCarta;
        }
        public beProveedoresRequisicionesCartas getProveedoresRequisicionesCarta(int pIdProveedorCarta)
        {
            var ProveedoresRequisicionesCarta = (new brProveedoresRequisicionesCartas()).traerProveedoresRequisicionesCartas_x_IdProveedorCarta(pIdProveedorCarta);

            return ProveedoresRequisicionesCarta;
        }
        public int saveProveedoresRequisicionesCarta(beProveedoresRequisicionesCartas pProveedoresRequisicionesCarta)
        {
            var ProveedoresRequisicionesCarta = (new brProveedoresRequisicionesCartas()).actualizarProveedoresRequisicionesCartas(pProveedoresRequisicionesCarta);

            return ProveedoresRequisicionesCarta;
        }
        public beManifiestoProveedoresClientes getManifiestoProveedoresClientes(int pIdManifiestoProveedorCliente)
        {
            var ManifiestoProveedorCliente = (new brManifiestoProveedoresClientes()).traerManifiestoProveedoresClientes_x_IdManifiestoProveedorCliente(pIdManifiestoProveedorCliente);

            return ManifiestoProveedorCliente;
        }
        public int saveManifiestoProveedorCliente(beManifiestoProveedoresClientes pManifiestoProveedoresClientes)
        {
            var ProveedoresRequisicionesCarta = (new brManifiestoProveedoresClientes()).guardarManifiestoProveedoresClientes(pManifiestoProveedoresClientes);

            return ProveedoresRequisicionesCarta;
        }
        public int saveEditManifiestoProveedorCliente(beManifiestoProveedoresClientes pManifiestoProveedoresClientes)
        {
            var ProveedoresRequisicionesCarta = (new brManifiestoProveedoresClientes()).actualizarManifiestoProveedoresClientes(pManifiestoProveedoresClientes);

            return ProveedoresRequisicionesCarta;
        }
        public int deleteManifiestoProveedorCliente(int pIdManifiestoProveedorCliente)
        {
            var ProveedoresRequisicionesCarta = (new brManifiestoProveedoresClientes()).eliminarManifiestoProveedoresClientes(pIdManifiestoProveedorCliente);

            return ProveedoresRequisicionesCarta;
        }
        public int saveManifiestosProveedores(beManifiestosProveedores pManifiestosProveedores)
        {
            var ManifiestosProveedores = (new brManifiestosProveedores()).actualizarManifiestosProveedores(pManifiestosProveedores);

            return ManifiestosProveedores;
        }
        public beManifiestosProveedores getManifiestosProveedores(int pIdManifiestoProveedor)
        {
            var ManifiestosProveedores = (new brManifiestosProveedores()).traerManifiestosProveedores_x_IdManifiestoProveedor(pIdManifiestoProveedor);

            return ManifiestosProveedores;
        }
        public int saveManifiestoProveedoresDeclaratorias(beManifiestoProveedoresDeclaratorias pManifiestoProveedoresDeclaratorias)
        {
            var ManifiestosProveedores = (new brManifiestoProveedoresDeclaratorias()).actualizarManifiestoProveedoresDeclaratorias(pManifiestoProveedoresDeclaratorias);

            return ManifiestosProveedores;
        }
        public beManifiestoProveedoresDeclaratorias getManifiestoProveedoresDeclaratorias(int pIdManifiestoProveedoresDeclaratorias)
        {
            var ManifiestosProveedores = (new brManifiestoProveedoresDeclaratorias()).traerManifiestoProveedoresDeclaratorias_x_IdManifiestoProveedorDeclaratoria(pIdManifiestoProveedoresDeclaratorias);

            return ManifiestosProveedores;
        }
        public beProveedoresRequisicionesInvitacion getProveedoresRequisicionesInvitacion(int pIdProveedoresRequisicionesInvitacion)
        {
            var ProveedoresRequisicionesInvitacion = (new brProveedoresRequisicionesInvitacion()).traerProveedoresRequisicionesInvitacion_x_IdProveedorRqInvitacion(pIdProveedoresRequisicionesInvitacion);

            return ProveedoresRequisicionesInvitacion;
        }
        public int saveProveedoresRequisicionesInvitacion(beProveedoresRequisicionesInvitacion pProveedoresRequisicionesInvitacion)
        {
            var ProveedoresRequisicionesInvitacion = (new brProveedoresRequisicionesInvitacion()).actualizarProveedoresRequisicionesInvitacion(pProveedoresRequisicionesInvitacion);

            return ProveedoresRequisicionesInvitacion;
        }

        public string getDependencia_x_IdDependencia(int pIdDependencia) {
            brDependencias d = new brDependencias();

            return d.traerDependencias_x_IdDependencia(pIdDependencia).Dependencia; 
        }
        ...
        public List<beProveedores> getAllProveedores(int IdRequisicion, int IdPartida)
        {
            brProveedores p = new brProveedores();

            var prov = p.getAllProveedores_x_IdRequisicion(IdRequisicion).ToList();

            return prov;
        }
        public List<beRequisicionConsulta> consultaRequisicionSegundaVuelta(int pIdDependencia)
        {
            beGenerica oGenerico = new beGenerica();
           
            oGenerico.entero = pIdDependencia;
            brRequisicionConsulta b = new brRequisicionConsulta();
            return b.listarRequisicionSegundaVueltaConsulta(oGenerico);

        }
        public List<beRequisicionConsulta> requisicionHistorial(int pIdDependencia, int pIdPerfil)
        {
            beGenerica oGenerico = new beGenerica();
            oGenerico.entero2 = pIdPerfil;
            oGenerico.entero = pIdDependencia;
            brRequisicionConsulta b = new brRequisicionConsulta();
            return b.listarRequisicionHistorial(oGenerico);
